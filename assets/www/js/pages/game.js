...
function loadGame(){
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#player1circle').removeAttr("src").attr("src",user.getGravatarURL(homeTeamObject.players[0].email_hash));
			
			$('#player2').text(homeTeamObject.players[1].name);
			$('#player2circle').removeAttr("src").attr("src",user.getGravatarURL(homeTeamObject.players[1].email_hash));
			
			$('#player3').text(homeTeamObject.players[2].name);
			$('#player3circle').removeAttr("src").attr("src",user.getGravatarURL(homeTeamObject.players[2].email_hash));
			
			$('#player4').text(awayTeamObject.players[0].name);
			$('#player4circle').removeAttr("src").attr("src",user.getGravatarURL(awayTeamObject.players[0].email_hash));
			
			$('#player5').text(awayTeamObject.players[1].name);
			$('#player5circle').removeAttr("src").attr("src",user.getGravatarURL(awayTeamObject.players[1].email_hash));
			
			$('#player6').text(awayTeamObject.players[2].name);
			$('#player6circle').removeAttr("src").attr("src",user.getGravatarURL(awayTeamObject.players[2].email_hash));
...
function undoShot(event){
	shot_id = event.data.shotId;
	player = event.data.playerNumber;
	...
	console.log('Undid shot, game is:'+JSON.stringify(thisGame));
...
function addShot(hitOrMiss, player_id, team_id, playerNumber){
	...
	//alert("new shotId " + shotId + " for player " + playerNumber);
	$('#' + playerNumber + 'undo').unbind('click').click({shotId:shotId, playerNumber:playerNumber}, undoShot);
	...
	if (bringBackCounter == 3){
		...
		window.plugins.ToastPlugin.show_short('Bring \'em back!');
		bringBackCounter = 0;
		if (shotsInRound == 6){
			newRound();
		}
